...
-- Create a function to increment interaction count
CREATE OR REPLACE FUNCTION increment_interaction_count(row_id UUID)
RETURNS INTEGER AS $$
DECLARE
  current_count INTEGER;
BEGIN
  SELECT interaction_count INTO current_count FROM chatbot_users WHERE id = row_id;
